...
| [05-replay-open-subaccount-escalation-supervisor-override](05-replay-open-subaccount-escalation-supervisor-override) | Deterministic replay — **human escalation** | Member `10003` is flagged for manager review. Replay can't resolve the expected form controls (a "Supervisor Approval Required" interstitial is shown instead), matches the `escalate` error handler, pauses, and raises an intervention on the mock operator console. Its `log.json` shows the full control-transfer state machine: a `control.transferred` event (`holder: "human"`) when the intervention is raised, `human.action` events (a POST request and a `framenavigated`) for the stand-in's own click on "Override (Supervisor)", and a second `control.transferred` (`holder: "automation"`, with `resumedBy`, `leftOnUrl`, and a `post-handoff-s6.png` screenshot) when it calls resume — the run then continues and reaches the confirmation screen. See REPORT.md §5 for the state machine. |
...
| [09-discovery-open-subaccount](09-discovery-open-subaccount) | **Real LLM discovery run** (GPT-4o) | Goal: "open a new sub-account for member 10002 with a $100 initial deposit named 'Emergency Fund' and reach the confirmation screen without submitting it." The agent works out the same search → detail → open sub-account → fill form → confirmation flow as the hand-authored `open-subaccount-to-confirmation`, entirely on its own, and produces `capabilities/open-subaccount-discovered.v1.0.json`. Succeeded on the first attempt with clean role/name locators and correctly parameterized inputs — no hand-editing, no prompt/recorder fix needed. See REPORT.md §2 for why this artifact and the hand-authored one both exist. |
